/* PlaySound UI polish: clean, colorful, no fade overlays. */
(function(){
  ...
  css.textContent=`
    ...
    .view,.category-card,.theme-card{animation:none!important;transition:transform .18s ease,box-shadow .18s ease,border-color .18s ease!important}
    .category-card:hover,.theme-card:hover{transform:translateY(-2px) rotate(-.1deg)}
    .category-card strong,.theme-card strong,.nav-button,.back-button,.secondary-button,.small-button{color:#b8a2d8!important}
    .nav-button:hover,.nav-button.active,.back-button:hover,.small-button:hover{color:#a98dcc!important}
    .brand-mark span,.brand-mark b{background:linear-gradient(135deg,#ff8fa3 0 20%,#ffd166 20% 40%,#8bd3dd 40% 60%,#a8d88b 60% 80%,#c5a7e8 80% 100%);-webkit-background-clip:text;background-clip:text;color:transparent!important}
  ...
  a.id='ps-bg-a';b.id='ps-bg-b';
  document.body.prepend(b,a);
  let active=a;
